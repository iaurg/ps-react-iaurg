...
if (enableWatchMode) {
  ...
  chokidar
    .watch([path.examples, path.components])
...
function generate(paths) {
  ...
  const componentData = getDirectories(paths.components).map(function (
    ...
    try {
      ...
      errors.push(
        `An error ocurred while attempting to generate metada for ${componentName}. ${error}`
      );
    }
  });
  writeFile(
...
function getExampleData(examplesPath, componentName) {
  ...
  return examples.map(function (file) {
    ...
    const info = parse(content);
    return {
      // By convention, componet name should match the filename.
